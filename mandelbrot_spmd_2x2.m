...
gridSize = 2000;   % Total grid size 2000 x 2000
blockSize = 1000;  % Each block will have 1000 x 1000 points (partitioning 2x2 for 500x500)
subBlockSize = 500;   % Each sub-block will have 500 x 500 points
maxIterations = 1000;
...
spmd
    ...
    x = linspace(xStart, xEnd, subBlockSize);
    y = linspace(yStart, yEnd, subBlockSize);
...
for i = 1:4
    ...
    rowStart = (row - 1) * subBlockSize + 1;
    rowEnd = row * subBlockSize;
    colStart = (col - 1) * subBlockSize + 1;
    colEnd = col * subBlockSize;
